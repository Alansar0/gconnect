    ...
    <div class="min-h-screen bg-[var(--bg-3)] text-[var(--text-1)] flex flex-col items-center font-sans relative">
        ...
        <header class="fixed top-0 left-0 right-0 z-40 bg-[var(--unique-card-bg)] border-b border-[var(--unique-card-border)] shadow-[0_0_15px_var(--shadow-accent)]">
            <div class="text-center py-4 relative">
                ...
                <h1 class="text-[var(--text-1)] text-xl font-semibold tracking-wide">{{ $displayName }}</h1>
            </div>
        ...
        <section id="sauraroView" class="flex-grow w-full px-6 py-4 overflow-y-auto mt-[110px] max-w-3xl transition-all duration-500">
            <div class="flex justify-center mb-4">
                <div class="relative bg-[var(--bg-2)] rounded-2xl p-3 border border-[var(--accent)]/50 shadow-[0_0_15px_var(--shadow-accent)] hover:shadow-[0_0_25px_var(--shadow-accent)] transition">
                    <img src="{{ Vite::asset('resources/images/kurakurai100.png')}}" alt="Makaranta Image"
                         class="w-[85vw] max-w-[600px] h-44 object-contain mx-auto drop-shadow-[0_0_12px_var(--accent)]/40">
                </div>
            </div>

            {{-- Audio Files List --}}
            @if(empty($files))
                <p class="text-center text-[var(--text-2)] mt-8">No audio files found for this course.</p>
            @else
                <ul class="space-y-3">
                    @foreach ($files as $index => $file)
                        <a href="{{ route('makaranta.sauraro', ['course' => $course ?? 'sharrindajjal', 'file' => $file]) }}"
                           class="bg-gradient-to-r from-[var(--bg-2)] to-[var(--unique-card-bg)] flex items-center justify-between rounded-2xl p-3 shadow-lg border border-[var(--accent)]/20">
                            <div class="flex items-center gap-3">
                                <button class="playButton w-10 h-10 rounded-full bg-[var(--accent)] flex items-center justify-center shadow-[0_0_12px_var(--shadow-accent)] hover:scale-105 transition">
                                    <i class="fas fa-play text-[var(--bg-3)]"></i>
                                </button>
                                <div class="text-left">
                                    <p class="text-[var(--text-1)] font-semibold text-base">Karatu {{ $index + 1 }}</p>
                                    <p class="text-[var(--accent)] text-sm">{{ $displayName }}</p>
                                </div>
                            </div>
                            <i class="material-icons text-[var(--accent)] !text-[40px]">chevron_right</i>
                        </a>
                    @endforeach
                </ul>
            @endif
